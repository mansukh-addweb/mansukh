...
class ShippingOptionUpdate extends Action implements CsrfAwareActionInterface
{
    ...
    public function execute()
    {
        ...
        try {
            $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/klarn.log');
            ...
            if ($shippingMethod = $data->getData('selected_shipping_option')) {
                ...
                $shippingMethodCode = $this->getShippingFromKSSCarrierClass($shippingMethod['id'], $logger);
     ...
     * @return string
     */
    private function convertShippingMethodCodeNew($shippingCode)
    {
        if (!strpos($shippingCode, '_')) return $shippingCode . '_' . $shippingCode;
        $parts = explode('_', $shippingCode);
        if ($parts) {
            return $parts[0] . '_' . $shippingCode;
        }

        return $shippingCode;
    }

    private function getShippingFromKSSCarrierClass($carrierClass, $logger) {
