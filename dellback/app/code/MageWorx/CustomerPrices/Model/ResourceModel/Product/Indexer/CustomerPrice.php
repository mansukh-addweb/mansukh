...
class CustomerPrice extends DefaultPrice
{
    ...
    protected function prepareMageWorxCatalogProductEntityDecimalCustomerPrice($entityIds, $customerIds)
    {
        ...
        $tableCustomerPrices = $this->getTable('mageworx_customerprices');
        $select              = $connection->select()->from($originalTable)->where(
            $originalTable . '.' . $linkField . ' IN(?)',
            $entityIds
        );

        $cond = 'customerprices.product_id = ' . $originalTable . '.' . $linkField;
        $select->joinLeft(
        ...
        $newTable = $this->getTable('mageworx_catalog_product_entity_decimal_customer_prices');
    ...
    protected function _movePriceDataToIndexTable($entityIds = null)
    {
        ...
        $indexTable = $this->getTable('mageworx_catalog_product_index_price');

        /* clean old data */
        $connection->delete(
            $indexTable,
            [
                'entity_id IN(?)' => $this->customerPricesResourceModel->getEntityIds($entityIds),
            ]
        );
    ...
    public function cleanAllTableReindex()
    {
        ...
        $table = $this->getTable('mageworx_catalog_product_entity_decimal_customer_prices');
        $connection->delete($table);

        $table = $this->getTable('mageworx_catalog_product_index_price');
        $connection->delete($table);
    ...
    /**
     * There is a bug in magento 2.1.2 with $attribute->isScopeGlobal() and need add check getIsGlobal() == 2
     *
     * @param $attribute
     * @return bool
     */
