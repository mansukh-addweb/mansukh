...
class Calculate extends AbstractHelper
{
    ...
    {
        /* check collection */
        if (!count($collection->getData()) || !$customerId) {
            return false;
        }

        foreach ($collection as $product) {
            if (!$product instanceof \Magento\Catalog\Model\Product || is_null($product->getPrice())) {
                return false;
            }
        }
    ...
    /**
     * @param array $customerPricesData
     * @param string $column
     * @return array
     */
    public function getColumnIds($customerPricesData, $column)
    {
        $ids = [];
        foreach ($customerPricesData as $elem) {
    ...
    public function getLinkField($class = ProductInterface::class)
    {
        if ($this->linkField === null) {
            $this->linkField = $this->metadataPool->getMetadata($class)->getLinkField();
        }
