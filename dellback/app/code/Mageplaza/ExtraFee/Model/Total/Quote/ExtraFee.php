...
class ExtraFee extends AbstractTotal
{
     ...
     * @return array
     */
    protected function checkApplyRule($quote, $area)
    {
        $ruleCollection = $this->ruleFactory->create()->getCollection()->setOrder('priority', 'ASC');
        $defaults       = [];
        $applyRule      = [];
    ...
    public function fetch(Quote $quote, Total $total)
    {
        ...
        $extraFee = $this->helper->getMpExtraFee($quote);
        if (empty($extraFee)) {
            $this->helper->setMpExtraFee($quote, $result, '4');
        ...
        switch ($type) {
            ...
            case FeeType::PERCENTAGE_OF_CART_TOTAL:
                ...
                if (in_array(CalculateOptions::TAX, $calculateOptions, false)) {
                    ...
                    $shippingAmount    = $quote->getShippingAddress()->getBaseShippingInclTax();
                }
     ...
     * @return array
     */
    protected function calculateAutoExtraFee($quote, $isFetch = false)
    {
    ...
    protected function fetchAutoFee(
        ...
        $label = Data::jsonDecode($rule->getLabels())[$quote->getStoreId()] ?: $rule->getName();
