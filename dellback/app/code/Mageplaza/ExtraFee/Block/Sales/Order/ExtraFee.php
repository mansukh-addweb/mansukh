...
class ExtraFee extends Template
{
    ...
    public function initTotals()
    {
        ...
        if (($parent->getSource()->getEntityType() === 'creditmemo'
                && $this->helper->isRefunded($order) && !$parent->getSource()->getId())
            || ($parent->getSource()->getEntityType() === 'invoice'
                && $this->helper->isInvoiced($order) && !$parent->getSource()->getId())
            || ($parent->getSource()->getEntityType() === 'creditmemo'
                && $parent->getSource()->getId() && $parent->getSource()->getId() !== $this->helper->isRefunded($order))
            || ($parent->getSource()->getEntityType() === 'invoice'
                && $parent->getSource()->getId() && $parent->getSource()->getId() !== $this->helper->isInvoiced($order))
        ) {
            return $this;
        }
